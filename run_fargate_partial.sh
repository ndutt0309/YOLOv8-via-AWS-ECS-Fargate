...
echo ""
read -p "Press ENTER when done testing to clean up expensive resources (ALB, tasks)... "


# ===== CLEANUP =====
echo "🧹 Cleaning up expensive resources"

# Scale ECS service to 0 tasks (no Fargate charges, service definition remains)
aws ecs update-service \
  --cluster "$CLUSTER_NAME" \
  --service "$SERVICE_NAME" \
...
echo "✅ ECS service scaled to 0 tasks (service and cluster kept)."

# Extra safety: stop any stray tasks
...
echo "✅ No running tasks remaining."

# Delete load balancer, listener & target group (to avoid hourly ALB cost)
if [ -n "$LISTENER_ARN" ] && [ "$LISTENER_ARN" != "None" ]; then
  aws elbv2 delete-listener --listener-arn "$LISTENER_ARN" > /dev/null || true
  echo "✅ Listener deleted"
else
  echo "ℹ️ Listener not found or already deleted."
fi
...
# ------------------------
# KEEP IAM role & network
# ------------------------
echo "ℹ️ Keeping IAM role: $ROLE_NAME"
echo "ℹ️ Keeping ECS cluster: $CLUSTER_NAME"
echo "ℹ️ Keeping VPC, subnets, route table, IGW, and security group: $VPC_ID / $SUBNET_ID / $SUBNET_ID2 / $SG_ID"
echo "ℹ️ Keeping CloudWatch alarms and dashboard: $DASHBOARD_NAME"
echo "ℹ️ Keeping task definitions and ECR image: $TASK_FAMILY / $ECR_IMAGE_URI"
